...
print(sys.path)
# we have add it into source root
...
def train(net_factory, prefix, end_epoch, base_dir,
    ...
    #tf.control_dependencies([]): train_op=tf.no_op(name='train')
    ...
    # high level tensor visualization
    ...
    try:
        ...
        for step in range(MAX_STEP):
            ...
            if (step+1) % display == 0:
                #acc = accurcy(cls_pred, labels_batch)
                total_loss,l2_loss,lr,acc = sess.run([total_loss_op,L2_loss_op,lr_op,accuracy_op],
                ...
                # landmark loss: %4f,
                print("%s : Step: %d/%d, accuracy: %3f, Total Loss: %4f ,lr:%f " % (datetime.now(), step+1,MAX_STEP, acc, total_loss, lr))
