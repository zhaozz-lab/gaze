...
def train(train_file,batch_size,train_layer,weight_path,filewriter_path):
...
def main(args):
    ...
    for i in range(15):
        ...
        train_lists=[h5py.File(train_list_name) for train_list_name in train_list]
        # train_files=[train_lists[0:5],train_lists[5:10],train_lists[10:]]
        train_files = [[train_lists[0]], [train_lists[1]], [train_lists[2]]]
        # train by three steps
        for train_file in train_files:
            # t=threading.Thread(target=train, args=(train_file, batch_size, train_layer, weight_path, filewriter_path))
            print('Parent process %s.' % os.getpid())
            p = Process(target=train, args=(train_file,batch_size,train_layer,weight_path,filewriter_path))
            p.start()
            p.join()
